...
const AUTH_TOKEN = token ? `Bearer ${token}` : null;
  const fetchUsers = async () => {
    try {
    const res = await fetch(`https://api.arcdatumcode.info/api/users`, {
  headers: {
...
setUsers(Array.isArray(data) ? data : data.data || []);
  ...
  const handleSave = async () => {
    try {
      if (editingUser) {
        await fetch(`https://api.arcdatumcode.info/api/api/users/${editingUser._id}`, {
          method: "PUT",
          headers: { "Content-Type": "application/json", "Authorization": `${AUTH_TOKEN}` },
          body: JSON.stringify(formData),
        });
      } else {
        await fetch(`https://api.arcdatumcode.info/api/api/auth/register`, {
          method: "POST",
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await fetch(`https://api.arcdatumcode.info/api/api/users/${id}`, { method: "DELETE" });
      fetchUsers();
    } catch (error) {
